...
new Promise(function(resolve) {
  ...
  // Новая функция setProgress для взаимодействия с прогресс-баром
  // При клике на него композиция «перематывается» на указанную позицию
  function setProgress(e) {
    let setTime = (progressLength) => {
      let setPosition = e.offsetX;
      globalPlayer.currentTime = globalPlayer.duration * setPosition / progressLength;
    };
    ...
    if (e.target.classList.contains('progress')) {
      ...
      if (currentItem === playingItem) {
        setTime(parseInt(window.getComputedStyle(e.target).width))
      }
    } else if (e.target.getAttribute('data-role') === 'progressbar') {
      let currentItem = e.target.parentNode.parentNode.parentNode.parentNode;
      if (currentItem === playingItem) {
        setTime(parseInt(window.getComputedStyle(e.target.parentNode).width))
      }
    }
  }
  ...
  // Прослушка клика на прогресс-бар для «перемотки» композиции
  results.addEventListener('click', setProgress);
  ...
  return new Promise(function(resolve, reject) {
    VK.api('audio.get', {version: 5.53}, function(response) {
      if (response.error) {
        reject(new Error(response.error.error_msg));
      } else {
